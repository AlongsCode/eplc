...
namespace eplast {
    ...
    inline auto node(Args&&... args) -> std::shared_ptr<T> {
    ...
    public:
        ...
        std::string generateCode() const override {
            ...
            std::string code = "for (" + val_name + " = 1; i <= " + end_var->generateCode() + "; " + val_name + "++) " + ASTStatement::generateCode() + "\n{";
            for (const auto& node : loopBlock) {
                code += node->generateCode() + "\n";
            }
            code += "}";
    ...
    public:
        ...
        std::string generateCode() const override {
            ...
            std::string code = "for (" + val_name + " = " + begin_var->generateCode() + "; i <= " + end_var->generateCode() + "; " + val_name + "+=(" + step_var->generateCode() + ")) " + ASTStatement::generateCode() + "\n{";
            for (const auto& node : loopBlock) {
                code += node->generateCode() + "\n";
            }
            code += "}";
    ...
    private:
        ...
        //{ 44,  OperatorInfo("fnLike", 6, OperatorType::Binary) },/*近似等于*/
    ...
    /*访问变量成员节点*/
    class ASTVariableMember :public ASTVariable
    {
        std::shared_ptr<ASTNode> m_obj{ nullptr };//节点对象
        //访问成员
        std::int16_t libid;
        std::int16_t structid;
    ...
    public:
        ...
        ASTVariableMember(const std::shared_ptr<ASTNode>& obj, std::int16_t structid, std::int32_t memberid)
            :m_obj(obj), libid(-2), structid(structid), memberid(memberid) {};

        std::string generateCode() const override {
            if (libid != -2)/*库类型*/
            {
                auto lib = eplcltype::LibDefinedDataType(libid, structid);
                return lib.getName() + lib.getMemberName(memberid);
            }
            return {};
            /*判断是否为数据类型*/
            auto obj = getDataType();
            if (obj->getObjType() == eplcltype::BaseCodeObj::type::Struct)
            {
                ...
                for (const auto& m : members) {
                    if (m->getID() == memberid)
                        return ASTVariable::generateCode() + "." + m->getName();
                }
            }
            debug_throw("未知的成员变量");
        }
    ...
    private:
        ...
        auto parseExpression() -> std::shared_ptr<ASTNode> {
            ...
            {
                /*        if (getNextToken() == EnumEplTokenType::FUNCTION_EXPRESSION_CLASS_END) {
                ...
                else  // 访问成员变量
                {
                    ...
                    if (_currentToken == EnumEplTokenType::FUNCTION_EXPRESSION_CLASS_END)//普通变量访问
                        return std::make_shared<ASTVariable>(findVar(variable));
                    else if (_currentToken == EnumEplTokenType::VARIABLE_MEMBER)//变量成员访问
                    {
                        ...
                        if (EplSystemId::IsLibDataType(structId))
                        {
                            ...
                            if (getNextToken() == EnumEplTokenType::FUNCTION_EXPRESSION_CLASS_END)
                                return  std::make_shared<ASTVariableMember>(std::make_shared<ASTVariable>(findVar(variable)), libId, structIdInLibId, memberId - 1);
                        }
                        else
                        {
                            auto token{ getNextToken() };
                            if (token == EnumEplTokenType::FUNCTION_EXPRESSION_CLASS_END)
                            {
                                return std::make_shared<ASTVariableMember>(std::make_shared<ASTVariable>(findVar(variable)), structId, memberId);
                            }
                    ...
                    {
                        nextTokenType();
                        auto ret = std::make_shared<ASTVariableIndex>(std::make_shared<ASTVariable>(findVar(variable)), parseExpression());
                        if (getNextToken() == EnumEplTokenType::FUNCTION_EXPRESSION_CLASS_END)
                            return ret;
                    }
        ...
        auto   parseFunctionCallStatement() -> std::shared_ptr<ASTStatement> {
            /*处理表达式*/
            auto methodId = m_ms.read<std::uint32_t>();
            auto spaceId = m_ms.read<std::int16_t>();
            auto flag = m_ms.read<std::int16_t>();
            auto code = m_ms.get_mfc_string();
            auto comment = m_ms.get_mfc_string();
            // 解析函数参数（如果有）
            std::vector<std::shared_ptr<ASTNode>> arguments;
            auto currentToken{ getNextToken() };
            ...
            /*变量成员函数和变量成员*/
                ...
                return std::make_shared<ASTClassFunctionCallStatement>(std::make_shared<ASTVariable>(findVar(variable)), findFun(methodId, spaceId), comment, spaceId, flag, arguments);
            }
    ...
    public://语句表达式解析
        ...
        auto parseFunctionCall() -> std::shared_ptr<ASTNode> {
            ...
            /*变量成员函数和变量成员*/
                ...
                return std::make_shared<ASTClassFunctionCall>(std::make_shared<ASTVariable>(findVar(variable)), findFun(methodId, spaceId), spaceId, flag, arguments);
            }
    ...
    public:
        ...
        auto parse(const std::vector<unsigned char>& inputTokens,
            ...
            chick_name(67177256);
#endif // _DEBUG
            eprogramutils::appio::ioput(L"正在转译子程序:",
                ansi2wstr(fun->getSapce()->getName()), L"::", ansi2wstr(fun->getName()), L" id:", fun->getID(), "\n\n");
            while (!m_ms.is_end()) {
